<script lang="ts">
	...
	import InvitationCreatedModal from '$lib/components/InvitationCreatedModal.svelte';
	...
	let invitationCreatedModal = $state(false);
	let invitationLink = $state('');

	function openInvitationCreatedModal(link: string) {
		invitationLink = link;
		invitationCreatedModal = true;
	}
	function closeInvitationCreatedModal() {
		invitationCreatedModal = false;
	}
	...
	async function createInvitation(): Promise<void> {
		if (!userId || !projectId) {
			console.error('Missing userId or projectId for invitation creation');
			return;
		}
		...
		const response = await fetch('?/createInvitationLink', {
			method: 'POST',
		...
		if (response.ok) {
			...
			openInvitationCreatedModal(JSON.parse(data)[1]);
		} else {
...
<InvitationCreatedModal
	open={invitationCreatedModal}
	onClose={closeInvitationCreatedModal}
	link={invitationLink}
/>
